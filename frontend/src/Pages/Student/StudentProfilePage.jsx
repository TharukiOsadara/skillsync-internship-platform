...
export default function StudentProfile() {
  ...
  const PwInput = ({ label, field, placeholder }) => {
    ...
    // Set autocomplete attribute for browser suggestion
    let autoComplete = "off";
    if (field === "current") autoComplete = "current-password";
    else if (field === "new" || field === "confirm") autoComplete = "new-password";
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
        <label style={{ fontSize: "10px", color: "#64748B", fontWeight: 600 }}>{label}</label>
        <div style={{ position: "relative" }}>
          <input
            ...
            style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: "8px", padding: "8px 34px 8px 11px", color: "#F1F5F9", fontSize: "12px", width: "100%", fontFamily: "'DM Sans',sans-serif", outline: "none", boxSizing: "border-box" }}
              ...
              {pwForm.confirmPassword && (
                <p style={{ fontSize: "10px", fontWeight: 700, margin: "0 0 12px", color: pwForm.newPassword === pwForm.confirmPassword ? "#4ADE80" : "#F87171", display: "flex", alignItems: "center", gap: "4px" }}>
                  <Ico size={11} stroke={pwForm.newPassword === pwForm.confirmPassword ? "#4ADE80" : "#F87171"}>
                    {pwForm.newPassword === pwForm.confirmPassword ? <polyline points="20 6 9 17 4 12" /> : <><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></>}
                  </Ico>
