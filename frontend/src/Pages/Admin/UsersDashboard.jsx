...
=======
  fullName:"",gmail:"",password:"",age:"",address:"",phoneNo:"",
  skills:"",education:"",experience:"",role:"Student",
>>>>>>> origin/CV-Builder
};
...
=======
  ...
  const openEditModal = (user) => {
    setEditUser(user); setUpdateAdminPassword(""); setError(""); setSuccess("");
    setForm({ fullName:user.fullName||"", gmail:user.gmail||"", password:user.password||"",
      age:user.age||"", address:user.address||"", phoneNo:user.phoneNo||"",
      skills:user.skills||"", education:user.education||"", experience:user.experience||"",
      role:user.role||"Student" });
  };
  ...
  const handleUpdateUser = async () => {
    ...
    if(!updateAdminPassword) return setError("Admin password is required.");
    try {
      const res=await fetch(`http://localhost:5000/users/${editUser._id}`,{
        method:"PUT", headers:authHeaders(),
        body:JSON.stringify({...form,adminPassword:updateAdminPassword}),
      });
      const data=await res.json();
      if(!res.ok) return setError(data.message||"Update failed.");
      setUsers(prev=>prev.map(u=>u._id===editUser._id?{...u,...form}:u));
      setEditUser(null); setUpdateAdminPassword("");
      setSuccess("User updated successfully.");
    } catch { setError("Server error while updating."); }
>>>>>>> origin/CV-Builder
  };
                  ...
                  {filtered.map((user,index)=>{
                    ...
                    return (
                      <tr key={user._id} className="border-b border-navy-800 hover:bg-navy-800/30 transition-colors">
                        ...
                        <td className="px-3 py-3 whitespace-nowrap">
                          <span className={(user.role||"Student")==="Admin"
                            ?"inline-block text-[10px] font-bold px-2 py-0.5 rounded-full bg-purple-400/10 text-purple-400 border border-purple-400/20"
                        ...
                        <td className="px-3 py-3 whitespace-nowrap">
                          <div className="flex gap-2">
                            <button onClick={()=>openEditModal(user)}
                              className="text-cyan-400 text-xs font-bold bg-cyan-400/8 border border-cyan-400/15 px-2 py-1 rounded-lg hover:bg-cyan-400/15 transition-all cursor-pointer">Update</button>
                            <button onClick={()=>setDeleteUserId(user._id)}
                              className="text-red-400 text-xs font-bold bg-red-400/8 border border-red-400/15 px-2 py-1 rounded-lg hover:bg-red-400/15 transition-all cursor-pointer">Delete</button>
>>>>>>> origin/CV-Builder
                          </div>
                        </td>
                      </tr>
...
=======
                  {filtered.length===0&&(
                    <tr><td colSpan={16} className="text-center text-slate-400 text-sm py-10">
...
=======
              {[["fullName","Full Name"],["gmail","Gmail"],["password","Password"],["age","Age"],["address","Address"],["phoneNo","Phone Number"],["skills","Skills"],["education","Education"],["experience","Experience"]].map(([key,label])=>(
                <div key={key} className="flex flex-col gap-1">
                  <label className="text-slate-400 text-xs">{label}</label>
...
=======
              ...
              <div className="flex flex-col gap-1">
                ...
                <input type="password" value={updateAdminPassword} onChange={e=>setUpdateAdminPassword(e.target.value)} placeholder="Enter admin password" className="input-field"/>
              </div>
            </div>
            <div className="flex gap-3 mt-5">
              <button onClick={()=>{setEditUser(null);setUpdateAdminPassword("");}} className="flex-1 py-3 bg-navy-800 border border-navy-700 text-slate-400 font-bold text-sm rounded-xl hover:border-slate-500 cursor-pointer transition-all">Cancel</button>
>>>>>>> origin/CV-Builder
