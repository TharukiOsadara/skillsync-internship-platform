  ...
  const fetchCV = async () => {
    try {
      ...
      if (data.success && data.data) {
        // CV exists in database - load all fields including profile image
        setFormData({
        ...
        // Load profile image from CV database (base64)
        if (data.data.profileImage) {
          setProfileImage(data.data.profileImage);
        }
        setHasExistingCV(true);
      } else {
        // No CV exists - pre-fill with user registration data as starting point
        if (user) {
          setFormData(prev => ({
            ...prev,
      ...
      // On error, still try to pre-fill with user data
      if (user) {
        setFormData(prev => ({
          ...prev,
          fullName: user.fullName || '',
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/cv`, {
        ...
        body: JSON.stringify({
          ...formData,
          profileImage, // Save base64 image to CV database
        }),
      });
      ...
      if (data.success) {
        setHasExistingCV(true);
        setMessage({ type: 'success', text: hasExistingCV ? 'CV updated successfully!' : 'CV created successfully!' });
      } else {
      ...
      <main style={{ flex: 1, padding: "32px", overflowY: "auto", minWidth: 0 }}>
        ...
        <div style={{
          ...
          <p style={{ fontSize: "12px", color: "#94A3B8", margin: 0 }}>
            <span style={{ color: "#22D3EE", fontWeight: 600 }}>Note:</span> Your CV data is stored separately from your account profile. Changes here won't affect your account details.
          </p>
        </div>
